...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        [SerializeField]
        private bool IsSizeFitter;

        [SerializeField]
        public bool IsPiovtReversal = false;
        ...
        /// </summary>
        public void ReloadData(bool forecMovePositionZero = true)
        {
            _reloadData = false;

            if(forecMovePositionZero)
                ScrollPosition = 0;
        ...
        public float GetCellOffsetPosition(int dataIndex, bool useSpacing, float cellOffset = 0)
        {
            ...
            return cellOffsetPosition;
        }

        public float GesOffset(float scrollerOffset = 0, float cellOffsetPosition = 0)
        {
            return -(scrollerOffset * ScrollRectSize) + cellOffsetPosition;
        }
        ...
        {
            var cellOffsetPosition = GetCellOffsetPosition(dataIndex, useSpacing, cellOffset);

            var newScrollPosition = 0f;

            // cache the offset for quicker calculation
            var offset = GesOffset(scrollerOffset, cellOffsetPosition);
        ...
        public void JumpToDataIndex(float newScrollPosition, float tweenTime, TweenType tweenType = TweenType.immediate, Action jumpComplete = null)
        {
            StartCoroutine(TweenPosition(tweenType, tweenTime, ScrollPosition, newScrollPosition, jumpComplete));
        }
        ...
        public float GetScrollPositionForCellViewIndex(int cellViewIndex, CellViewPositionEnum insertPosition)
        {
            ...
            if (cellViewIndex < 0 || (cellViewIndex == 0 && insertPosition == CellViewPositionEnum.Before))
            {
                return 0;
            }
            else if (cellViewIndex < _cellViewOffsetArray.Count)
            {
            ...
            else
            {
                ...
                return 1 < _cellViewOffsetArray.Count ? _cellViewOffsetArray[_cellViewOffsetArray.Count - 2] : _cellViewOffsetArray[_cellViewOffsetArray.Count - 1];
            }
        }
        ...
        /// </summary>
        public SmallList<EnhancedScrollerCellView> _activeCellViews = new SmallList<EnhancedScrollerCellView>();
        ...
        void Awake()
        {
            ...
            if (scrollDirection == ScrollDirectionEnum.Vertical)
            {
                ...
                _container.pivot = new Vector2(0.5f, IsPiovtReversal ? 0 : 1f);
            }
            else
            {

...
namespace Dev.Data
{
    public class Table
    {
            ...
            for(int y = start.Row, indexY = 0; y <= end.Row; ++y , ++indexY)
            {
                for(int x= start.Column,indexX = 0 ; x <= end.Column; ++x, ++indexX)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //todo typename에 따라 default 값 세팅하게 수정
                            text = "0";
                        }
                        data[indexY - 1, indexX] = text;
                    }
                }
            }
        ...
        public Table(string tableName,int rowCount, int colCount,string[] titles, string[,] data)
        {
            ...
            {
                TableUtility.SplitDataNameAndTypeName(titles[i],rowCount,out m_dataNames[i],out m_typeNames[i]);
            }
